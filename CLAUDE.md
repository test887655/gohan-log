...
- 写真（任意）＋メニュー名（列は note。画面の表記だけ「メニュー名」にした）＋日時＋朝/昼/夜/間食の区分
  ＋「なに」（食べ物/飲み物/その他。列は category。必ず選ぶ）
  - カードではメニュー名のすぐ上に「● 食べ物」のように札（.category-tag）で出す。
...
- 朝・昼・夜の3食を記録した日は、ボーナス10ポイント。1日1回だけ。
  ...
  - 数えるのは「写真つき」で「食べ物」の記録だけ（points.js の checkMealBonus）。
    写真なしや、飲み物・その他は記録はできるがボーナスに入れない。
...
- meals (id, user_id, photo_path, note, place, eaten_at, meal_type, created_at)
  ...
  - category は 'food' / 'drink' / 'other'（check 制約 meals_category_check）。
    既定は 'food'。列を足す前の記録はぜんぶ食べ物として扱う
