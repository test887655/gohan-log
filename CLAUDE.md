...
- 食材リストの見出しにある吹き出しから、やりとりの画面を開く。
  そうだんの見出しには、冷蔵庫（食材リストへ）・吹き出し・お茶碗（ごはんへ）を並べる。
  まん中の吹き出しは押すためのものではなく、今どこを開いているかの目印。
  食材リストを開くとヘッダーの冷蔵庫に色が付くのと同じ見た目（aria-pressed="true"）にする。
  そうだんのあいだはヘッダーの冷蔵庫を引っ込める。冷蔵庫が2つ出ていると
  どちらで戻るのか分かりにくいため、戻り道は見出しの1つだけにする
